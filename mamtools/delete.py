...
__all__ = ['delete', 'history', 'collapse', 'merge_faces', 'transforms']
...
def delete():
    ...
    for comp in s.itercomps():
        if not comp:
            ...
            cmds.polyDelEdge(list(comp))
        elif comp.is_vert():
            cmds.polyDelVertex(list(comp))
        else:
            cmds.delete(str(comp))
...
def merge_faces():
    ...
    s = mampy.selected()
    if not s or not isinstance(next(s.itercomps()), mampy.MeshPolygon):
        return logger.warn('No polygon selected.')

    faces = mampy.SelectionList()
    for comp in s.itercomps():
        border_verts = comp.to_vert(border=True)
        internal_edges = comp.to_edge(internal=True)
        cmds.polyDelEdge(list(internal_edges), cv=False)
...
def transforms(translate=False, rotate=False, scale=False):
    ...
    transforms = [str(dp) for dp in s.iterdags() if dp.type == MFn.kTransform]
